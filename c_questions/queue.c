...
struct my_queue {
  int count;
  STAILQ_HEAD(list_type, entry_type) head;
};

struct entry_type {
  void *something;
  STAILQ_ENTRY(entry_type) entry_next;
};
...
int main()
{
  ...
  for (i = 0; i < 10; i++)
  {
    ...
    STAILQ_INSERT_TAIL( &entry_list->head, new_entry, entry_next );
    entry_list->count++;
  }
  ...
  STAILQ_FOREACH( current_entry, &entry_list->head, entry_next )
  {
